...
func ListFile(c *gin.Context) {
	// 创建OSSClient实例。
	client, err := oss.New("https://oss-accelerate.aliyuncs.com", "1", "2")
	if err != nil {
		HandleError(err)
	}
	// 获取存储空间。
...
func UploadFile(c *gin.Context, localfile string) (img string){
	client, err := oss.New("oss-cn-shanghai.aliyuncs.com", "1", "2")
	if err != nil {
		// HandleError(err)
		fmt.Println(err)
	}
...
func GetStorageList(c *gin.Context) {
	client, err := oss.New("oss-cn-shenzhen.aliyuncs.com", "1", "2")
	if err != nil {
		// HandleError(err)
		fmt.Println(err)
	}
